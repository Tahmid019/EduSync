...
def run_lip_sync(checkpoint_path, face_path, audio_path):
    print("start=====>>")
    print(os.path)
    os.chdir('Wav2Lip')
    print(os.curdir)
...
def upload_file():
    ...
    try:
        ...
        final_video_path_sub = os.path.join(PROCESSED_FOLDER, f"{os.path.splitext(file.filename)[0]}_{utc_str}_final_sub.mp4")   #1 in a billion case......if same random generated for user in same time
        ...
        subprocess.run([
            ...
            '--outfile', final_video_path_sub
        ])
        # time.sleep(180)
        
        # src_file = '/results/result_voice.mp4'
        # dest_lang
        ...
        # final_video_path_merge = os.path.join(PROCESSED_FOLDER, f"{os.path.splitext(file.filename)[0]}_{utc_str}_final_merge.mp4")   #1 in a billion case......if same random generated for user in same time
        ...
        # run_lip_sync(checkpoint_path=checkpoint, face_path=final_video_path_merge, audio_path=final_audio_path)
        ...
        return jsonify({
            ...
            'video_url': f"/processed/{os.path.splitext(file.filename)[0]}_{utc_str}_final_sub.mp4",
