...
def upload_file():
    ...
    try:
        ...
        # print(f"vid: {os.path.splitext(silent_video_path)[0]}")
        # print(f"aud: {os.path.splitext(final_audio_path)[0]}")
            ...
            if merge[i][2] == 1:
                ...
                try:
                    subprocess.run([
                        ...
                        '--outfile', vid_chunks[i]
                    ])
                    print("Subprocess completed successfully")
                    final.append(vid_chunks[i])
                    print(f"==>>{final}")
                except EnvironmentError as subprocess_err:
                    print(f"[Subprocess error] : {subprocess_err}")
                    print("Calling Merge , after subprocess_error ...")
                    try:
                        merge_audio_with_silent_video(vid_chunks[i], aud_chunks[i], vid_chunks[i])
                        print("Merge completed successfully")
                        final.append(vid_chunks[i])
                        print(f"==>>{final}")
                    except EnvironmentError as merge_err:
        ...
        # Return the URL of the final video and transcription file
        # final_video_url = f"/processed/{os.path.basename(final_clip)}"
        transcription_file_url = f"/transcriptions/{os.path.basename(transcription_file_path)}"
        print(final_clip)
        ...
        return jsonify({
                ...
                'video': f"/video/{final_clip}",
                # 'transcription_url': transcription_file_url,
                'transcription': f"{utc_str}_transcription.txt"
            }), 200
