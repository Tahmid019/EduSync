...
def up_f(file_path, sr_lang, dest_lang, utc_str):
...
def upload_file():
    ...
    vid_chunks = split_video(file_path, merge, f"{os.path.splitext(file.filename)[0]}_{utc_str}")
    print(vid_chunks)
    #=======
    ...
    for i in range(0, num_seg):
        vid = vid_chunks[i]
        print(f"{i}: vid_prefix={vid}")
        try:
            video_url, transcription_text = up_f(vid, sr_lang, dest_lang, utc_str) #, merge[i][2]
    ...
    for lol in range (0,3):
        try:
            clips = [VideoFileClip(video) for video in final_vid_url]
            print(clips)
        except Exception as cl:
            print(f"[CLIP ERROR: {cl}]")
            continue
