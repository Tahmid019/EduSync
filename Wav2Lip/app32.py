...
def upload_file():
    ...
    try:
        ...
        # ])
        # print("===>>>>outside subprocess")
        # count = 10000
        # while(True):
        #     print("=========>>>>>inside while loop=======<<<<")
        #     time.sleep(10)
        #     if(find('results/', 'result_voice.mp4')) or count==0:
        #         print(f"{count}")
        #         break
        #     else:
        #         count-=1
        ...
        #result_voice.mp4
        final_video_url = f"/processed/{os.path.basename(final_video_path)}" # {os.path.basename(final_video_path)}
        ...
        return jsonify({
            ...
            'transcription': translated_text,  #old: transcription_text    new: translated_tes
        }), 200
    except Exception as e:
