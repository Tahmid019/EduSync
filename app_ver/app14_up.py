...
def signup():
    ...
    db_cursor.execute('CREATE DATABASE IF NOT EXISTS lipsync')
    db_cursor.execute('USE lipsync')
    query = 'SELECT * FROM `userregdetails` WHERE u_mail = `' + data['u_mail'] + '` AND u_pass = `' + data['u_pass'] + '`'
    db_cursor.execute(query)
    # db_cursor.execute(db_insert, ("abc", "def", "abdf@gmail.com", "1234"))
    print(db_cursor.rowcount, "Record displayed")
    return jsonify(db_cursor), 123456
