...
def upload_file():
    ...
    try:
        ...
        transcription1 = transcribe_audio(audio_file_path)
        ...
        transcription_file_path = os.path.join(TRANSCRIPTIONS_FOLDER, 'transcription1.txt')  #f"{os.path.splitext(file.filename)[0]}_transcription.txt"
        ...
        transcription_file_url = f"/transcriptions/transcription1.txt"     #{os.path.basename(transcription_file_path)}
        return jsonify({
...
def register_user():
    ...
    u_fname = data.get('u_fname')
    u_lname = data.get('u_lname')
    u_mail = data.get('u_mail')
    u_pwd = data.get('u_pwd')
    connection = get_db_connection()
    cursor = connection.cursor()
    cursor.execute('INSERT INTO userregdetails (u_fname, u_lname, u_mail, u_pwd) VALUES (%s, %s, %s, %s)', (u_fname, u_lname, u_mail, u_pwd))
    connection.commit()
    connection.close()
...
def login_user():
    ...
    connection = get_db_connection()
    cursor = connection.cursor()
    cursor.execute('SELECT * FROM userregdetails WHERE u_mail = %s AND u_pwd = %s', (u_mail, u_pwd))
