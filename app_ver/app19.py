...
def demouser():
    ...
    db_cursor.execute('CREATE DATABASE IF NOT EXISTS lipsync')
    db_cursor.execute('USE lipsync')
    db_cursor.execute('CREATE TABLE IF NOT EXISTS `lipsync`.`demo_usr_review` (`duid` INT NOT NULL AUTO_INCREMENT , `du_ip_address` VARCHAR(100) NOT NULL , `du_sr_lang` TEXT NOT NULL , `du_dest_lang` TEXT NOT NULL , `du_vnum` INT NOT NULL , `du_lip_q` INT NOT NULL , `du_tr_q` INT NOT NULL , `du_aud_q` INT NOT NULL , `du_all_q` INT NOT NULL , `du_review_timestamp` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP , PRIMARY KEY (`duid`), UNIQUE (`du_mail`))')
    res = db_cursor.execute('SELECT * FROM `demo_usr_review` WHERE du_ip = '+ data['usrip'])
    if res:
        query = 'INSERT INTO `userregdetails` (du_ip_address) VALUES (%s, %s, %s, %s)'
        db_cursor.execute(query, (data['u_fname'], data['u_lname'], data['u_mail'], data['u_pass']))
        # db_cursor.execute(db_insert, ("abc", "def", "abdf@gmail.com", "1234"))
        mydb.commit()
        print(db_cursor.rowcount, "Record inserted")
        ...
        query = 'INSERT INTO `userregdetails` (du_ip) VALUES (%s, %s, %s, %s)'
        db_cursor.execute(query, (data['u_fname'], data['u_lname'], data['u_mail'], data['u_pass']))
        # db_cursor.execute(db_insert, ("abc", "def", "abdf@gmail.com", "1234"))
        mydb.commit()
        print(db_cursor.rowcount, "Record inserted")
