...
def process_audio_chunks(audio, silence_thresh=-40, min_silence_len=1000):
    ...
    for start_time, end_time in silent_ranges:
        if start_time > start:
            non_silent_chunks.append((start, start_time))
        start = end_time
    if start < len(audio) / 1000:
        non_silent_chunks.append((start, len(audio) / 100))
...
def upload_file():
    ...
    try:
        ...
        non_silent_chunks = process_audio_chunks(audio)
        ...
        for start, end in non_silent_chunks:
            ...
            transcription_loop = transcribe_audio("chunk.wav")
            if not transcription_loop:
                print(f"Skipping chunk {start}-{end}: Failed to transcribe audio")
                translated_audio_segments.append((start, chunk))
                continue

            translated_text, detected_lang = translate_text_file(transcription_loop, dest_lang)
            if not translated_text:
        ...
        # Merge translated segments back into their original positions
        for start, segment in translated_audio_segments:
            final_audio = final_audio.overlay(segment, position=start*100)
