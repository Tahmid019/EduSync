...
#     try:
#         # detected_lang = GoogleTranslator.detect(input_text).lang
#         translated = GoogleTranslator(source='en', target=dest_language).translate(input_text)
...
#         # Detect source language and translate transcription
...
#         translated_audio_path = text_to_speech_from_text(translated_text, lang=dest_lang, pitch_change=-2)
...
def convert_mp4_to_wav(mp4_file_path):
    try:
        print("inside convert_mp4_to_wav")
        base_name = os.path.splitext(os.path.basename(mp4_file_path))[0]
        print("inside convert_mp4_to_wav__2")
        wav_file_path = os.path.join(PROCESSED_FOLDER, f"{base_name}.wav")
        print("inside convert_mp4_to_wav__3")
        wav_file_path = ffmpeg.input(mp4_file_path).output(wav_file_path, format='wav').run(overwrite_output=True)
        print("inside convert_mp4_to_wav__4")
        print(f"Conversion complete: {wav_file_path}")
        return wav_file_path, 11
    except ffmpeg.Error as e:
        print(f"ffmpeg error: {e.stderr.decode()}"), 10
    return None
...
def detect_non_silent_regions(audio, silence_thresh=-80, min_silence_len=1000):
...
def upload_file():
    ...
    file_path = os.path.join(UPLOAD_FOLDER, file.filename) ##needs change here
    file.save(file_path)
    ...
    try:
        ...
        audio_file_path = convert_mp4_to_wav(file_path)
        if audio_file_path is None:
            return jsonify({'error': 'Failed to convert video to audio'}), 5000
